...
import RenderExploreAllBlogs from "./components/listArticles"
import RenderTop10Articles from "./components/top10Articles"
import Pagination from "./components/pagination"
import Footer from "../../components/footer"
import { FontAwesomeIcon } from "@fortawesome/react-fontawesome"
import { faArrowUpAZ, faArrowDownZA  } from "@fortawesome/free-solid-svg-icons"
import RenderCarouselBanner from "./components/newBlogs"
import RenderNewBlogs from "./components/newBlogs"
import RenderSidebarCategories from "./components/sidebarCategory"


function LandingPage () {
    const dispatch = useDispatch()
    const { isLogoutLoading, loading, loadingTop10, currentPage, totalPage, articles, username, categories ,favorites, top10_fav, newBlog} = useSelector(state => {
        return {
        ...
        <div>
            ...
            <div className="flex flex-col flex-wrap gap-5 justify-between py-20">
                    ...
                    <div className="flex flex-row w-full h-auto gap-5 justify-center">
                        <div className="flex flex-col">
                            <label className="swap">
                                ...
                                />
                                <button className="swap-on text-bold btn btn-slate-200">Sort A to Z</button>
                                <button className="swap-off text-bold btn btn-slate-200">Sort Z to A</button>
                            </label>
                        </div>
                        <select 
                ...
                <div class="flex flex-col w-full lg:flex-row h-full mt-8 pb-2 ">                
                    ...
                    <div class="bg-gray-200 rounded-lg flex flex-grow-0 card w-[450px] carousel carousel-vertical rounded-box place-items-start flex-wrap gap-5 py-5 mt-5 h-fit">
                        <a className="font-bold text-lg px-5">
                            {
                                valueCategory?.name == "allCategory" || valueCategory?.name == "" 
                                ? "Pick a category to see the popular articles"
